...
def train():
    ...
    # 如果是重新开始训练，'w' 会覆盖旧日志；如果是断点续训，可以用 'a'
    log_file = open(log_path, "w", encoding="utf-8")

    # 在 open 文件后立即写入(记录部分重要参数)
    log_file.write(f"Training Settings: epochs={epochs}, lr={lr}, device={device}\n")
    log_file.write("-" * 50 + "\n")
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(dataloader):
            ...
            s_normal = ((data['B'].to(device) + 1.0) / 2.0)
            ...
            opt_enh.zero_grad()
            # 使用 .detach() 截断梯度，Enh 训练时不影响 Dec 的权重
            # 重新获取最新的 R_low
            with torch.no_grad():
        ...
        # 打印日志到控制台
        log_str = f"Epoch [{epoch+1}/{epochs}] | Dec Loss: {epoch_dec_loss/len(dataloader):.6f} | Enh Loss: {epoch_enh_loss/len(dataloader):.6f}"
        print(log_str)
        
        # --- 新增：写入日志到文件 ---
        log_file.write(log_str + "\n")
        log_file.flush()  # 确保实时写入硬盘，防止程序崩溃导致日志丢失
    ...
    # 训练结束
    log_file.close()
    print("Training finished. Log saved to:", log_path)
